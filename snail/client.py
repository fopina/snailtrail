...
class Client:
    ...
    def join_competitive_races(self, snail_id: int, race_id: int):
        """join mission race - signature is generated by `sign_race_join`"""
        signature = self.web3.sign_race_join(snail_id, race_id)
        r = self.gql.join_competitive_races(snail_id, race_id, self.web3.wallet, signature)
        if r.get('status') != 1:
            raise ClientError('unknown status', r)
    ...
    def tournament(self, address, tournament_id=None):
        return types.Tournament(self.gql.tournament(address, tournament_id=tournament_id))
